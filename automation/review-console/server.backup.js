...
class ReviewConsoleServer {
    ...
    async getDrafts(req, res) {
        console.log('📥 getDrafts endpoint called');
        
        try {
            const draftsPath = path.join(this.contentDir, 'drafts');
            console.log(`📁 Drafts directory path: ${draftsPath}`);
    ...
    async findArticleById(id) {
        ...
        for (const folder of folders) {
            try {
                const filePath = path.join(this.contentDir, folder, `${id}.json`);
                const content = await fs.readFile(filePath, 'utf8');
    ...
    async moveArticle(id, fromFolder, toFolder, article) {
        const fromPath = path.join(this.contentDir, fromFolder, `${id}.json`);
        ...
        console.log(`Moved article ${id} from ${fromFolder} to ${toFolder}`);
    }
